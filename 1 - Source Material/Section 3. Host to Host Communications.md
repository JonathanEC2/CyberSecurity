...
A wireless access point is a device that enables wireless network connectivity for devices, acting as a bridge between a wired network and a wireless in the same area
...
The Open System Interconnect model is a standard of the International Organization for Standardization. Its a general purpose framework that standardizes how computer communicate with each other over a network. A layer serves the layer above it and it is served by a layer below it
...
TCP/IP is the main protocol used stack used in computer operations today. It is used to transfer data in production networks
