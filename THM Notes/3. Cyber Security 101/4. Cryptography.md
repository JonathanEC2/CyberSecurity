...
Permissions must be set correctly to use a private SSH key, otherwise your SSH client will ignore the file with a warning. nly the owner should be able to read or write to the private key (600 or stricter). <span style="color:rgb(255, 192, 0)">ssh -i {PRIVATE_KEY} user@host</span> is how you specify a key
